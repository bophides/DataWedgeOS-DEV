...
class ConfigurationActivity : AppCompatActivity(), Observer, View.OnClickListener, CompoundButton.OnCheckedChangeListener {
    ...
    private var scanList: ArrayList<Scan>? = null;

    private val REQUEST_SAVE_FILE = 0x1337;
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val scans = getIntent().getBundleExtra("scanBundle").getSerializable("allScans") as ArrayList<Scan>;
        if (scans != null) {
            scanList = scans;
        }
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (resultCode === RESULT_OK && attr.data != null) {
            if (requestCode === REQUEST_SAVE_FILE) {
                try {
                    val os: OutputStream = contentResolver.openOutputStream(data?.data);
                    os?.write(getCSVFile().toByteArray());
    ...
    private fun getCSVFile(): String {
        var csvFileContent: String = "";

        for (scan: Scan in scanList!!) {
            csvFileContent += scan.symbology;
            csvFileContent += ",";
            csvFileContent += scan.dateTime;
            csvFileContent += "\n";
        }
    ...
    override fun onClick(view: View?) {
        ...
        when(view?.id)
        {
            ...
            {
                try {
                    //val reader = CSVReader(FileReader("yourfile.csv"))
                    //var nextLine: Array<String?>
                    Log.d("JIMMY", "Loggind data...");
                    Log.d("JIMMY", scanList?.toString());
                    var csvFileContent: String = "";
                    ...
                    val sendIntent: Intent = Intent().apply {
                        ...
                        putExtra(Intent.EXTRA_TEXT, csvFileContent);
                        intent.putExtra(Intent.EXTRA_TITLE, "scans.csv");

                        type = "text/csv"
                    }

                    val shareIntent = Intent.createChooser(sendIntent, null)
                    startActivityForResult(shareIntent, REQUEST_SAVE_FILE)
                }
                catch (e: Exception) {
                    e.printStackTrace()
                }
